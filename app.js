...
function renderJobs(){let q=jobSearch.value.toLowerCase();let arr=jobs.filter(j=>canSeeJob(j)&&JSON.stringify(j).toLowerCase().includes(q));jobList.innerHTML=arr.length?arr.map(j=>`<div class="item ${j.id===selectedJobId?"active":""}" data-job-id="${esc(j.id)}" draggable="true"><div class="title">${esc(j.name)}</div><div class="meta">Earthwork: ${esc(j.dirt_start||"—")} → ${esc(j.dirt_end||"—")}</div><div class="meta">${requests.filter(r=>canSeeRequest(r)&&isActiveRequest(r)&&r.job_id===j.id).length} Active Work</div><div class="dragHint">Drag to reorder</div></div>`).join(""):`<div class="muted">No jobs yet.</div>`;document.querySelectorAll("[data-job-id]").forEach(el=>{el.onclick=()=>{if(window.__draggingJob)return;selectedJobId=el.dataset.jobId;renderAll();openJobDrawer(selectedJobId);};el.addEventListener("dragstart",e=>{window.__draggingJob=el.dataset.jobId;el.classList.add("dragging");e.dataTransfer.effectAllowed="move";});el.addEventListener("dragend",()=>{el.classList.remove("dragging");setTimeout(()=>window.__draggingJob=null,50);});el.addEventListener("dragover",e=>e.preventDefault());el.addEventListener("drop",async e=>{e.preventDefault();const from=window.__draggingJob,to=el.dataset.jobId;if(from&&to&&from!==to)await reorderJobsByDrop(from,to);});});}
...
function equipmentThumbUrl(type, description){
  const text = String((type || "") + " " + (description || "")).toLowerCase();

  let query = "construction equipment";

  if(text.includes("dozer")) query = "bulldozer construction";
  else if(text.includes("excavator") || text.includes("trackhoe") || text.includes("mini x")) query = "construction excavator";
  else if(text.includes("skid")) query = "skid steer loader";
  else if(text.includes("roller") || text.includes("compactor")) query = "road roller compactor";
  else if(text.includes("water truck")) query = "construction water truck";
  else if(text.includes("dump truck")) query = "construction dump truck";
  else if(text.includes("articulated")) query = "articulated dump truck";
  else if(text.includes("lowboy")) query = "lowboy trailer construction";
  else if(text.includes("backhoe")) query = "backhoe loader";
  else if(text.includes("loader")) query = "wheel loader construction";
  else if(text.includes("sweeper")) query = "street sweeper truck";
  else if(text.includes("survey")) query = "survey equipment tripod";

  // Remote generic stock-style thumbnail by equipment type.
  // Not actual company equipment; used only for visual identification.
  return "https://source.unsplash.com/240x160/?" + encodeURIComponent(query);
}
...
function renderOperationsBoard(){
  ...
  operationsBoard.innerHTML=`
    <div class="boardTools">
      ...
      <div class="muted">Tip: Use each card's Move To dropdown instead of dragging when the list is long. Unassigned: ${unassignedCount}</div>
    </div>
    <div class="boardGrid">
      ${Object.entries(grouped).map(([job,eqs])=>`
        <div class="card boardCol" data-board-job="${esc(job)}">
          <div class="boardColHeader">
            <h3>${esc(job)}</h3>
            <span class="badge">${eqs.length} equipment</span>
          </div>
          <div class="boardColBody">
            ${eqs.length?eqs.map(eq=>`
              <div draggable="true" data-equipment-id="${esc(eq.id)}" class="equipmentCard" style="background:${equipmentTypeColor(eq.equipment_type)};border:${equipmentStatusBorder(eq.status)}">
                <div class="equipVisual">
                  <img class="equipThumb" src="${equipmentThumbUrl(eq.equipment_type, eq.name)}" alt="${esc(eq.equipment_type || "Equipment")}" loading="lazy" referrerpolicy="no-referrer">
                  <div class="equipIcon">${equipmentTypeIcon(eq.equipment_type)}</div>
                </div>
                <b>${esc(eq.name)}</b>
            ...
            `).join(""):`<div class="muted">No equipment assigned.</div>`}
          </div>
        </div>
      `).join("")}
    </div>`;
  ...
  document.querySelectorAll("[data-board-job]").forEach(col=>{
    ...
    col.addEventListener("drop",async e=>{
      ...
      if(!id)return;
      let target=col.dataset.boardJob;
  ...
  if(searchBox){
    searchBox.oninput=()=>{
      ...
      document.querySelectorAll(".equipmentCard").forEach(card=>{
