...
function normalizeJob(j){return {id:j.id,sort_order:j.sort_order??9999,name:j.name||"Untitled Job",address:j.address||"",owner:j.owner||"",site_contact:j.site_contact||"",earthwork_start:j.earthwork_start||"",earthwork_end:j.earthwork_end||"",storm_drain_start:j.storm_drain_start||"",storm_drain_end:j.storm_drain_end||"",sewer_start:j.sewer_start||"",sewer_end:j.sewer_end||"",water_start:j.water_start||"",water_end:j.water_end||"",dropbox_link:j.dropbox_link||"",notes:j.notes||"",updated_at:j.updated_at};}
...
function jobPhaseOverview(j){
  return `<div class="jobPhaseOverview">
    <div class="jobPhaseOverviewBox phaseEarthwork">
      <div class="jobPhaseOverviewTitle">EARTHWORK</div>
      <div class="jobPhaseDates">Start: ${esc(j.earthwork_start||"—")}<br>End: ${esc(j.earthwork_end||"—")}</div>
    </div>
    <div class="jobPhaseOverviewBox phaseStorm">
      <div class="jobPhaseOverviewTitle">STORM DRAIN</div>
      <div class="jobPhaseDates">Start: ${esc(j.storm_drain_start||"—")}<br>End: ${esc(j.storm_drain_end||"—")}</div>
    </div>
    <div class="jobPhaseOverviewBox phaseSewer">
      <div class="jobPhaseOverviewTitle">SEWER</div>
      <div class="jobPhaseDates">Start: ${esc(j.sewer_start||"—")}<br>End: ${esc(j.sewer_end||"—")}</div>
    </div>
    <div class="jobPhaseOverviewBox phaseWater">
      <div class="jobPhaseOverviewTitle">WATER</div>
      <div class="jobPhaseDates">Start: ${esc(j.water_start||"—")}<br>End: ${esc(j.water_end||"—")}</div>
    </div>
  </div>`;
}
...
const deptConfigs={
  ...
  Earthwork:{steps:["job","priorityDue","personnel","description"],desc:"Earthwork Work Details",placeholder:"What earthwork needs done? Include area, grade/sequence, and notes."},
...
function refreshJobDrawer(){let j=currentJob();jobDrawerTitle.textContent=j?"Job Overview":"New Job";jobNameInput.value=j?.name||"";jobAddressInput.value=j?.address||"";jobOwnerInput.value=j?.owner||"";jobSiteContactInput.value=j?.site_contact||"";earthworkStartInput.value=dateOnly(j?.earthwork_start||"");
earthworkEndInput.value=dateOnly(j?.earthwork_end||"");
