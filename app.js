...
function workTitle(r){let note=String(r.description||"").trim();if(note){let first=note.split("\n").find(x=>x.trim())||note;return first.length>70?first.slice(0,70)+"…":first;}if(r.department==="Mobilization"&&r.equipment_name)return"Move: "+r.equipment_name;if(r.department==="Mechanic"&&r.equipment_name)return"Maintenance: "+r.equipment_name;if(r.equipment_name)return r.equipment_name;return(r.department||"Work")+" Request";}
...
function requestLink(requestId){
  return appBaseUrl() + "?request=" + encodeURIComponent(requestId);
}

function notificationRecipientsForRequest(r){
  const dept = String(r.department || "").toLowerCase();
  const person = String(r.personnel || "").toLowerCase();

  let people = personnelList.filter(p => {
    if(person){
      return String(p.full_name || "").toLowerCase() === person;
    }
    return String(p.department || "").toLowerCase() === dept;
  });
...
async function queueNotification(payload){
  try{
    let res = await apiFetch(rest("notification_queue"),{
      method:"POST",
    ...
    if(!res.ok){
      console.warn("Notification queue failed", await res.text());
    }
  }catch(e){
    console.warn("Notification queue error", e);
  }
}

async function queueAssignmentNotifications(r){
  const link = requestLink(r.id);
  const recipients = notificationRecipientsForRequest(r);

  if(!recipients.length){
    await addUpdate("No email/phone recipients found for notification.","notification");
    return;
  }

  const subject = "New " + (r.department || "Work") + " Request — " + workTitle(r);
  ...
  await addUpdate("Notification queued for " + recipients.map(r=>r.name || r.email || r.phone).join(", ") + ".","notification");
}

async function queueCompletionNotification(r){
  if(!r.created_by_email) return;

  const link = requestLink(r.id);
  const subject = "Work Completed — " + workTitle(r);

  const message =
    subject + "\\n\\n" +
    "Completed by: " + actorEmail() + "\\n" +
  ...
  await queueNotification({
    ...
    recipient_email: r.created_by_email,
  ...
  await addUpdate("Completion notification queued for requester: " + r.created_by_email + ".","notification");
}
...
async function openRequestFromUrl(){
  ...
  if(found){
    await openWorkDrawer(requestId);
  }else{
    alert("Request link loaded, but this request was not found or you do not have permission to view it.");
  }
}
...
function renderJobs(){let q=jobSearch.value.toLowerCase();let arr=jobs.filter(j=>canSeeJob(j)&&JSON.stringify(j).toLowerCase().includes(q));jobList.innerHTML=arr.length?arr.map(j=>`<div class="item ${j.id===selectedJobId?"active":""}" data-job-id="${esc(j.id)}" draggable="true"><div class="title">${esc(j.name)}</div><div class="meta">Dirt: ${esc(j.dirt_start||"—")} → ${esc(j.dirt_end||"—")}</div><div class="meta">${requests.filter(r=>canSeeRequest(r)&&isActiveRequest(r)&&r.job_id===j.id).length} Active Work</div><div class="dragHint">Drag to reorder</div></div>`).join(""):`<div class="muted">No jobs yet.</div>`;document.querySelectorAll("[data-job-id]").forEach(el=>{el.onclick=()=>{if(window.__draggingJob)return;selectedJobId=el.dataset.jobId;renderAll();openJobDrawer(selectedJobId);};el.addEventListener("dragstart",e=>{window.__draggingJob=el.dataset.jobId;el.classList.add("dragging");e.dataTransfer.effectAllowed="move";});el.addEventListener("dragend",()=>{el.classList.remove("dragging");setTimeout(()=>window.__draggingJob=null,50);});el.addEventListener("dragover",e=>e.preventDefault());el.addEventListener("drop",async e=>{e.preventDefault();const from=window.__draggingJob,to=el.dataset.jobId;if(from&&to&&from!==to)await reorderJobsByDrop(from,to);});});}
...
const deptConfigs={
  ...
  Dirt:{steps:["job","priorityDue","personnel","description"],desc:"Dirt Work Details",placeholder:"What dirt work needs done? Include area, grade/sequence, and notes."},
  Utility:{steps:["job","priorityDue","personnel","description"],desc:"Utility Work Details",placeholder:"What utility work needs done? Include utility type, location, conflict, and timing."},
  ...
  Mechanic:{steps:["equipment","mechanic","job","priorityDue","personnel","description"],desc:"Maintenance Note",placeholder:"Describe the problem clearly. Example: leaking hydraulic fluid near left track, machine still running but weak."},
  Mobilization:{steps:["equipment","job","priorityDue","personnel","description"],desc:"Move / Mobilization Details",placeholder:"Where does this equipment need to go? Include pickup location, destination, timing, and trailer/lowboy notes."}
};
...
function optionPersonnel(dept){let filtered=personnelList.filter(p=>!dept||String(p.department||"").toLowerCase()===String(dept).toLowerCase());return `<option value="">Department-wide</option>${filtered.map(p=>`<option value="${esc(p.full_name)}">${esc(p.full_name)}${p.role?" / "+esc(p.role):""}</option>`).join("")}`;}
...
function renderWorkForm(existing=null){
  ...
  for(const step of cfg.steps){
    ...
    if(step==="description") html+=`<div class="stepBox"><div class="stepTitle">${n++}. Details</div><label>${esc(cfg.desc)}</label><textarea id="f_description" placeholder="${esc(cfg.placeholder)}"></textarea><label>Dropbox Link</label><input id="f_dropbox" placeholder="Optional override"></div>`;
  }
...
function getWorkForm(){const dept=workDepartment.value;const eq=document.getElementById("f_equipment")?equipmentList.find(e=>e.id===f_equipment.value):null;const job=document.getElementById("f_job")?jobs.find(j=>j.id===f_job.value):null;return{department:dept,job_id:job?.id||null,job:job?.name||"",priority:document.getElementById("f_priority")?f_priority.value:"Medium",status:document.getElementById("f_status")?f_status.value:"New",due_at:document.getElementById("f_due")&&f_due.value?new Date(f_due.value).toISOString():null,personnel:document.getElementById("f_personnel")?f_personnel.value:"",equipment_id:eq?.id||null,equipment_name:eq?.name||"",mechanic_severity:document.getElementById("f_mechSeverity")?f_mechSeverity.value:null,can_operate:document.getElementById("f_canOperate")?f_canOperate.value:null,dropbox_link:document.getElementById("f_dropbox")?f_dropbox.value:"",description:document.getElementById("f_description")?f_description.value:""};}
function clearWorkForm(){selectedId=null;workDepartment.value="";renderWorkForm(null);setWorkMode("new");}
function setWorkMode(mode){sendWorkBtn.classList.toggle("hidden",mode!=="new");clearWorkBtn.classList.toggle("hidden",mode!=="new");saveWorkBtn.classList.toggle("hidden",mode==="new");completeWorkBtn.classList.toggle("hidden",mode==="new");addUpdateBtn.classList.toggle("hidden",mode==="new");deleteWorkBtn.classList.toggle("hidden",mode==="new");}
...
async function sendWork(){if(!canEdit())return alert("No permission.");let data=getWorkForm();if(!data.department)return alert("Select who this is assigned to.");if(data.department==="Mechanic"&&!data.equipment_id)return alert("Select equipment for the maintenance request.");if(data.department==="Mobilization"&&!data.equipment_id)return alert("Select equipment for the mobilization request.");if(!data.description)return alert("Add request details.");let payload={...data,created_by:session.user.id,created_by_email:actorEmail(),updated_by:session.user.id,updated_at:new Date().toISOString()};let res=await apiFetch(rest("REQUESTS"),{method:"POST",headers:headers({"Prefer":"return=representation"}),body:JSON.stringify(payload)});let out=await res.json();if(!res.ok)return alert(JSON.stringify(out));let created=normalizeReq(out[0]);requests.unshift(created);selectedId=created.id;let msg=created.department==="Mechanic"?`Maintenance request created. Equipment: ${created.equipment_name||"Not listed"}. Severity: ${created.mechanic_severity||"Normal"}. Can operate: ${created.can_operate||"Unknown"}. Note: ${created.description||""}`:created.department==="Mobilization"?`Mobilization request created. Equipment: ${created.equipment_name||"Not listed"}. Job: ${created.job||"No specific job"}. Note: ${created.description||""}`:`Work request created. Assigned to ${created.department}${created.personnel?" / "+created.personnel:" department-wide"}.`;await addUpdate(msg,"created");await queueAssignmentNotifications(created);await loadRequests();status("Work request sent and saved.");closeWorkDrawer();clearWorkForm();renderAll();}
...
async function applyMobilizationCompletion(r){
  if(!r || r.department !== "Mobilization") return;

  if(!r.equipment_id){
    await addUpdate("Mobilization completed, but no equipment was attached so the Operations Board was not changed.","system");
    return;
  }

  const targetJob = r.job || (jobs.find(j=>j.id===r.job_id)?.name) || null;

  if(!targetJob){
    await addUpdate("Mobilization completed, but no destination job/project was selected so the Operations Board was not changed.","system");
    return;
  }

  let res = await apiFetch(rest("equipment","?id=eq."+r.equipment_id),{
    method:"PATCH",
    headers:headers({"Prefer":"return=representation"}),
    body:JSON.stringify({
      current_job: targetJob,
      status: "Active"
    })
  });
...
function renderOperationsBoard(){let grouped={};jobs.forEach(j=>grouped[j.name]=[]);grouped["SHOP"]=grouped["SHOP"]||[];grouped["UNASSIGNED"]=grouped["UNASSIGNED"]||[];equipmentList.forEach(eq=>{let key=eq.current_job||"UNASSIGNED";if(!grouped[key])grouped[key]=[];grouped[key].push(eq);});operationsBoard.innerHTML=`<div class="boardGrid">${Object.entries(grouped).map(([job,eqs])=>`<div class="card boardCol" data-board-job="${esc(job)}"><h3 style="margin-top:0">${esc(job)}</h3>${eqs.length?eqs.map(eq=>`<div draggable="true" data-equipment-id="${esc(eq.id)}" class="equipmentCard" style="background:${equipmentTypeColor(eq.equipment_type)};border:${equipmentStatusBorder(eq.status)}"><b>${esc(eq.name)}</b><div class="meta">${esc(eq.equipment_type||"Equipment")}${eq.equipment_number?" • "+esc(eq.equipment_number):""}</div><div class="meta">Status: ${esc(eq.status||"Active")}</div>${eq.assigned_foreman?`<div class="meta">Foreman: ${esc(eq.assigned_foreman)}</div>`:""}</div>`).join(""):`<div class="muted">No equipment assigned.</div>`}</div>`).join("")}</div>`;document.querySelectorAll("[data-equipment-id]").forEach(card=>{card.addEventListener("dragstart",()=>window.__dragEquipmentId=card.dataset.equipmentId);card.addEventListener("dragend",()=>window.__dragEquipmentId=null);});document.querySelectorAll("[data-board-job]").forEach(col=>{col.addEventListener("dragover",e=>e.preventDefault());col.addEventListener("drop",async e=>{e.preventDefault();let id=window.__dragEquipmentId;if(!id)return;let target=col.dataset.boardJob;let res=await apiFetch(rest("equipment","?id=eq."+id),{method:"PATCH",headers:headers({"Prefer":"return=representation"}),body:JSON.stringify({current_job:target==="UNASSIGNED"?null:target})});if(!res.ok)return alert("Could not move equipment.");await loadEquipment();renderOperationsBoard();status("Equipment moved.");});});}
...
function renderPersonnelManageList(){personnelManageList.innerHTML=personnelList.length?personnelList.map(p=>`<div class="feedItem"><b>${esc(p.full_name)}</b><div class="meta">${esc(p.department||"No department")} • ${esc(p.role||"No role")}</div><div class="meta">${esc(p.email||"")}${p.phone?" • "+esc(p.phone):""}</div><div class="toolbar" style="margin-top:8px"><button class="secondary" data-edit-personnel="${esc(p.id)}">Edit</button><button class="danger" data-disable-personnel="${esc(p.id)}">Deactivate</button></div></div>`).join(""):"No personnel added yet.";document.querySelectorAll("[data-edit-personnel]").forEach(btn=>btn.onclick=()=>{let p=personnelList.find(x=>x.id===btn.dataset.editPersonnel);if(!p)return;personnelId.value=p.id||"";personnelName.value=p.full_name||"";personnelDepartment.value=p.department||"";personnelRole.value=p.role||"";personnelEmail.value=p.email||"";personnelPhone.value=p.phone||"";personnelNotes.value=p.notes||"";});document.querySelectorAll("[data-disable-personnel]").forEach(btn=>btn.onclick=async()=>{if(!confirm("Deactivate this person?"))return;let res=await apiFetch(rest("personnel","?id=eq."+btn.dataset.disablePersonnel),{method:"PATCH",headers:headers({"Prefer":"return=representation"}),body:JSON.stringify({active:false})});if(!res.ok)return alert(await res.text());await loadPersonnel();renderPersonnelManageList();});}
...
function renderEquipmentManageList(){let q=(equipmentSearch.value||"").toLowerCase();let filtered=equipmentList.filter(eq=>JSON.stringify(eq).toLowerCase().includes(q));equipmentManageList.innerHTML=filtered.length?filtered.map(eq=>`<div class="feedItem"><b>${esc(eq.name)}</b><div class="meta">${esc(eq.equipment_type||"No type")} • ${esc(eq.equipment_number||"")}</div><div class="meta">Status: ${esc(eq.status||"Active")} • Site: ${esc(eq.current_job||"UNASSIGNED")}</div><div class="toolbar" style="margin-top:8px"><button class="secondary" data-edit-equipment="${esc(eq.id)}">Edit</button><button data-maintenance-equipment="${esc(eq.id)}">Maintenance Request</button></div></div>`).join(""):"No equipment found.";document.querySelectorAll("[data-edit-equipment]").forEach(btn=>btn.onclick=()=>{let eq=equipmentList.find(x=>x.id===btn.dataset.editEquipment);if(!eq)return;equipmentEditId.value=eq.id||"";equipmentNameInput.value=eq.name||"";equipmentNumberInput.value=eq.equipment_number||"";equipmentTypeInput.value=eq.equipment_type||"";equipmentStatusInput.value=eq.status||"Active";equipmentCurrentJobInput.value=eq.current_job||"";equipmentForemanInput.value=eq.assigned_foreman||"";equipmentNotesInput.value=eq.notes||"";});document.querySelectorAll("[data-maintenance-equipment]").forEach(btn=>btn.onclick=async()=>{let eq=equipmentList.find(x=>x.id===btn.dataset.maintenanceEquipment);if(!eq)return;equipmentDrawer.classList.add("hidden");await openWorkDrawer(null);workDepartment.value="Mechanic";renderWorkForm();setTimeout(()=>{if(document.getElementById("f_equipment"))f_equipment.value=eq.id;},0);});}
...
if(session){startSessionAutoRefresh();loadProfile().then(async()=>{setViews();await loadAll();await openRequestFromUrl();}).catch(logout);}else setViews();
