...
function renderPersonnelOptionsForDepartment(){
  ...
  const selected = personnel.value;

  const filtered = personnelList.filter(p=>{
    if(!dept) return true;
    return String(p.department || "").toLowerCase() === String(dept).toLowerCase();
  });
  ...
  // Keep existing value while editing if still available.
  if(selected && filtered.some(p=>p.full_name === selected)){
    personnel.value = selected;
  }else if(selected && !filtered.some(p=>p.full_name === selected)){
    personnel.value = "";
  }
}
