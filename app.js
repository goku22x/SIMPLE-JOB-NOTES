...
function setViews(){
  ...
  topButtons.classList.toggle("hidden",!session);
  if(session){
    signedInName.textContent=session.user.email;
    roleBadge.textContent=profile?.role||"unknown";

    assignWorkBtn.disabled=!canEdit();

    const adminOnly = isAdmin();
    newJobBtn.classList.toggle("hidden",!adminOnly);
    personnelBtn.classList.toggle("hidden",!adminOnly);
    equipmentBtn.classList.toggle("hidden",!adminOnly);

    newJobBtn.disabled=!adminOnly;
    personnelBtn.disabled=!adminOnly;
    equipmentBtn.disabled=!adminOnly;
  }
}}
...
async function savePersonnel(){if(!isAdmin())return alert("Admin only.");let payload={full_name:personnelName.value.trim(),department:personnelDepartment.value,role:personnelRole.value.trim(),email:personnelEmail.value.trim(),phone:personnelPhone.value.trim(),notes:personnelNotes.value.trim(),active:true};if(!payload.full_name)return alert("Enter a full name.");let res;if(personnelId.value)res=await apiFetch(rest("personnel","?id=eq."+personnelId.value),{method:"PATCH",headers:headers({"Prefer":"return=representation"}),body:JSON.stringify(payload)});else res=await apiFetch(rest("personnel"),{method:"POST",headers:headers({"Prefer":"return=representation"}),body:JSON.stringify(payload)});let out=await res.json().catch(()=>[]);if(!res.ok)return alert(JSON.stringify(out));clearPersonnelForm();await loadPersonnel();renderPersonnelManageList();status("Personnel saved.");}
