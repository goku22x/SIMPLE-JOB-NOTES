...
async function sendRequest(){if(!canEdit())return alert("No permission.");let data=getRequestForm();if(!data.department)return alert("Select who this is assigned to.");if(data.department==="Mechanic"&&!data.equipment_id)return alert("Select equipment for the maintenance request.");if(data.department==="Mobilization"&&!data.equipment_id)return alert("Select equipment for the mobilization request.");if(!data.description)return alert(data.department==="Mechanic"?"Add a maintenance note.":"Add request description.");let payload={...data,created_by:session.user.id,created_by_email:actorEmail(),updated_by:session.user.id,updated_at:new Date().toISOString()};let res=await apiFetch(rest("REQUESTS"),{method:"POST",headers:headers({"Prefer":"return=representation"}),body:JSON.stringify(payload)});let out=await res.json();if(!res.ok)return alert(JSON.stringify(out));let created=normalizeReq(out[0]);requests.unshift(created);selectedId=created.id;await addUpdate(created.department==="Mechanic"?"Maintenance request created. Equipment: "+(created.equipment_name||"Not listed")+". Severity: "+(created.mechanic_severity||"Normal")+". Can operate: "+(created.can_operate||"Unknown")+". Note: "+(created.description||""):created.department==="Mobilization"?"Mobilization request created. Equipment: "+(created.equipment_name||"Not listed")+". Job: "+(created.job||"No specific job")+". Note: "+(created.description||""):"Work request created. Assigned to "+created.department+".","created");await loadRequests();status("Work request sent and saved.");drawer.classList.add("hidden");selectedId=null;clearRequestForm();renderAll();}
...
function clearPersonnelForm(){
  personnelId.value="";
  personnelName.value="";
  personnelDepartment.value="";
  personnelRole.value="";
  personnelEmail.value="";
  personnelPhone.value="";
  personnelNotes.value="";
}
