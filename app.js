...
if(typeof closeJobDrawer !== "function"){
  function closeJobDrawer(){
    const d=document.getElementById("jobDrawer");
    if(d) d.classList.add("hidden");
  }
}


// Emergency fallback for progressive form.
if(typeof updateProgressiveForm !== "function"){
  function updateProgressiveForm(){
    try{
      const assigned=document.getElementById("assignedStep");
      const remaining=document.getElementById("remainingSteps");
      const dept=document.getElementById("department");

      if(!assigned || !remaining || !dept) return;
