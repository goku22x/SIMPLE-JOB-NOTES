...
async function loadPersonnel(){
  ...
  personnel.innerHTML='<option></option>'+personnelList.map(p=>`<option value="${esc(p.full_name)}">${esc(p.full_name)}${p.department?" — "+esc(p.department):""}${p.role?" / "+esc(p.role):""}</option>`).join("");
}
...
function equipmentTypeColor(type){
  ...
  if(type.includes("excavator")) return "#fb923c";
  if(type.includes("truck")) return "#60a5fa";
          ...
          ${equipment.length ? equipment.map(eq=>`
            ...
            >
              ...
              <div style="font-size:12px;margin-top:4px">
                ${esc(eq.equipment_type || "Equipment")}
              </div>
